...
| **Best-in-class** | + NCBI pataa (local BLASTp) + NCBI nr (web API) + Lens.org | ⚠ Run `scripts/run_patent_blastp.py` |
...
All de novo candidates generated by `scripts/generate_denovo_novel_amps.py` (seed=42, 30 targets found in 15,315 attempts) were re-audited against the expanded 51,503-sequence DB.
