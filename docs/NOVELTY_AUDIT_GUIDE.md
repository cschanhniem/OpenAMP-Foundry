...
After deduplication: **37,005 unique clean standard-AA sequences (5–100 aa)**
...
| [ESCAPE](https://doi.org/10.7910/DVN/C69MCD) | 21,409 AMPs (80k total incl negatives) | **Included** ✅ | Harvard Dataverse; 3,542 clean AMP sequences extracted |
...
| Result | Count |
|--------|-------|
| | **34,555-seq DB** (pre-ESCAPE) | **37,005-seq DB** (incl ESCAPE) |
|---|---|---|
| HIGH_CONFIDENCE_NOVEL + CLEAR | 23 | **19** |
| RELATED_NOVEL + CLEAR | 7 | **11** |
| POSSIBLE_PATENT_RISK | 0 | **0** |
| SYNTHESIS_HOLD | 0 | **0** |

ESCAPE added 2,450 new unique sequences and downgraded 4 candidates from HCN → RELATED_NOVEL
(all still CLEAR, all still approved for wet lab).

Top 12 selected for external predictor submission (`outputs/denovo_top12_final.fasta`):
8 × HIGH_CONFIDENCE_NOVEL + 4 × RELATED_NOVEL (top by pipeline ensemble score, all CLEAR).
