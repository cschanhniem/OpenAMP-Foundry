...
| External predictor ensemble adapters (CAMPR4, AMPScanner, dbAMP, AntiCP2, Macrel) | Independent second opinions on activity; required for scientific credibility. Manual web-submission checklist exists at `outputs/external_predict_checklist.md`. Macrel v1.6.0 CLI has known ONNX scoring bug — all sequences (including magainin-2, LL-37) misclassified as NAMP (documented PR #77); use web server at big-data-biology.org/software/macrel | Medium (checklist done; submissions pending) |
