...
Basis:
- Pipeline AUROC = 0.8047 (95% CI: 0.71–0.89) vs composition-matched UniProt decoys (PR #66 dedup fix; 43 unique AMPs, n=87 total; previous 0.8086 pre-dedup, 0.8164 pre-Trp-bonus)
...
(a) Only 6 scaffold families are in the actual synthesis pool (SEED-003, 005, 006, 007, 008, 009;
    SEED-001, 002, 004 excluded at earlier gates); the per-scaffold analysis below uses actual pilot
    panel observed serum_stability values, not seed-level averages.
...
The AUROC 0.8047 (phase3 synthesis gate: 0.7846) is measured on a small 43+44 demo dataset, not validated against the full
...
- Internal benchmark AUROC = 0.8047 (n=87, bootstrap n=2000; phase3 synthesis gate = 0.7846; PR #66 dedup fix)  
